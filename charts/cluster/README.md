...
The chart supports [CNPG's declarative logical replication](https://cloudnative-pg.io/documentation/current/declarative_publication_management/)
through Publication and Subscription CRDs.

**Requirements:**
- Publisher cluster must have `wal_level: logical` set in `cluster.postgresql.parameters`
- Table schemas must exist on the subscriber before the subscription becomes active
- The user connecting from subscriber to publisher must have REPLICATION privilege

**Publisher cluster example:**
```yaml
cluster:
  postgresql:
    parameters:
...
**Subscriber cluster example:**
```yaml
subscriptions:
  - name: from_publisher       # Note: Use underscores, not hyphens
    dbname: app
...
logicalReplication:
  externalClusters:
    - name: publisher
      ...
      port: 5432
      username: postgres        # Must have REPLICATION privilege
      database: app
      sslMode: require
      passwordSecret:
...
- The external cluster connection typically uses the `postgres` superuser since it has REPLICATION privilege
- See the [CloudNativePG documentation](https://cloudnative-pg.io/documentation/current/declarative_subscription_management/) for advanced configuration options
...
| logicalReplication | object | `{"externalClusters":[]}` | Logical replication external cluster connections. Defines connections to external clusters for logical replication subscriptions. |
...
| publications | list | `[]` | Logical replication publications. Publications define what data to replicate from this cluster. Requires wal_level: logical in cluster.postgresql.parameters. See: https://cloudnative-pg.io/documentation/current/declarative_publication_management/ |
...
| subscriptions | list | `[]` | Logical replication subscriptions. Subscriptions receive data from a publication on another cluster. Requires an externalCluster entry in logicalReplication.externalClusters. See: https://cloudnative-pg.io/documentation/current/declarative_subscription_management/ |
